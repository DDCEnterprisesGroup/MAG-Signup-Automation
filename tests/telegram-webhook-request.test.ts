...
const headerFixture = "synthetic-telegram-secret";
...
test("missing and invalid Telegram secrets are rejected before routing", async () => {
  ...
  assert.equal((await handleTelegramWebhookRequest(request(), headerFixture, route)).status, 401);
  assert.equal((await handleTelegramWebhookRequest(request("wrong"), headerFixture, route)).status, 401);
  assert.equal((await handleTelegramWebhookRequest(request(headerFixture), "", route)).status, 401);
...
test("a valid Telegram secret reaches routing; a failure returns a retryable error", async () => {
  let routedId = -1;
  const ok = await handleTelegramWebhookRequest(request(headerFixture), headerFixture, async (update) => { routedId = update.update_id; });
  assert.equal(ok.status, 200);
  assert.equal(routedId, 37);
  const originalError = console.error;
  console.error = () => undefined;
  try {
    const failed = await handleTelegramWebhookRequest(request(headerFixture), headerFixture, async () => { throw new Error("synthetic failure"); });
